...
class CGCT_Discrete_Model : public scgms::CBase_Filter, public scgms::IDiscrete_Model {

	private:
		// last step time; NaN until initialized by outer code
		double mLast_Time = std::numeric_limits<double>::quiet_NaN();
		// model parameters
		gct_model::TParameters mParameters;
	...
	public:
		CGCT_Discrete_Model(scgms::IModel_Parameter_Vector* parameters, scgms::IFilter* output);
		virtual ~CGCT_Discrete_Model() = default;
