...
/*************************************************
 * Samadi modified model implementation          *
 *************************************************/

CSamadi_Modified_Discrete_Model::CSamadi_Modified_Discrete_Model(scgms::IModel_Parameter_Vector *parameters, scgms::IFilter *output) :
	CBase_Filter(output),
	mParameters(scgms::Convert_Parameters<samadi_modified_model::TParameters>(parameters, samadi_modified_model::default_parameters.vector)),
	mEquation_Binding{
		{ mState.Q1,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dQ1,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.Q2,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dQ2,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.Gsub,  std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dGsub,   this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.S1,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dS1,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.S2,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dS2,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.I,     std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dI,      this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.x1,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dx1,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.x2,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dx2,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.x3,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dx3,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.D1,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dD1,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.D2,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dD2,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.DH1,   std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dDH1,    this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.DH2,   std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dDH2,    this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.E1,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dE1,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.E2,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dE2,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.TE,    std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dTE,     this, std::placeholders::_1, std::placeholders::_2) },
		{ mState.Z,     std::bind<double>(&CSamadi_Modified_Discrete_Model::eq_dZ,      this, std::placeholders::_1, std::placeholders::_2) },
	}
{
	mState.lastTime = -1;
	...
	mSleep_Quality.Add_Uptake(0, std::numeric_limits<double>::max(), 0.0); // 0.0 == "not sleeping"
}

double CSamadi_Modified_Discrete_Model::eq_dQ1(const double _T, const double _X) const
{
	...
	const double FR = Gt >= Gthresh_FR ? 0.003 * (Gt - Gthresh_FR) * VgBW : 0;

	// MOD: glucose appearance is modulated by Z compartment
	const double UG = (1.0 + mState.Z) * (mState.D2 / mParameters.tmaxG +mState.DH2 / (mParameters.tmaxG / 2.0));

	// MOD: first glucose flux (disappearance) is modulated by Z compartment (e.g.; sleep quality)
	const double ret = -(1 + mParameters.alpha * mState.E2 * mState.E2 + mState.Z) * mState.x1 * _X + mParameters.k12 * mState.Q2 - F01C - FR + UG + EGP0BW * (1 - mState.x3);

	return ret;
}

double CSamadi_Modified_Discrete_Model::eq_dQ2(const double _T, const double _X) const
{
...
double CSamadi_Modified_Discrete_Model::eq_dGsub(const double _T, const double _X) const
{
	const double VgBW = mParameters.Vg * mParameters.BW;
...
double CSamadi_Modified_Discrete_Model::eq_dS1(const double _T, const double _X) const
{
...
double CSamadi_Modified_Discrete_Model::eq_dS2(const double _T, const double _X) const
{
	return mState.S1 / mParameters.tmaxi - _X / mParameters.tmaxi;
}

double CSamadi_Modified_Discrete_Model::eq_dI(const double _T, const double _X) const
{
...
double CSamadi_Modified_Discrete_Model::eq_dx1(const double _T, const double _X) const
{
	return -mParameters.ka1 * _X + mParameters.kb1 * mState.I;
}

double CSamadi_Modified_Discrete_Model::eq_dx2(const double _T, const double _X) const
{
	return -mParameters.ka2 * _X + mParameters.kb2 * mState.I;
}

double CSamadi_Modified_Discrete_Model::eq_dx3(const double _T, const double _X) const
{
	return -mParameters.ka3 * _X + mParameters.kb3 * mState.I;
}

double CSamadi_Modified_Discrete_Model::eq_dD1(const double _T, const double _X) const
{
...
double CSamadi_Modified_Discrete_Model::eq_dD2(const double _T, const double _X) const
{
	return mState.D1 / mParameters.tmaxG - _X / mParameters.tmaxG;
}

double CSamadi_Modified_Discrete_Model::eq_dDH1(const double _T, const double _X) const
{
...
double CSamadi_Modified_Discrete_Model::eq_dDH2(const double _T, const double _X) const
{
	return mState.DH1 / (mParameters.tmaxG / 2.0) - _X / (mParameters.tmaxG / 2.0);
}

double CSamadi_Modified_Discrete_Model::eq_dE1(const double _T, const double _X) const
{
	// MOD: cut heart rate to zero if the actual rate is below steady level
	//		this would otherwise cause a negative flux, which may result in unwanted glucose appearance
	const double HRdelta = std::max(0.0, mParameters.HRbase - mHeart_Rate.Get_Recent(_T * scgms::One_Minute));
...
double CSamadi_Modified_Discrete_Model::eq_dE2(const double _T, const double _X) const
{
...
double CSamadi_Modified_Discrete_Model::eq_dTE(const double _T, const double _X) const
{
	const double e1fact = std::pow(mState.E1 / (mParameters.a * mParameters.HRbase), mParameters.n);
	const double fE1 = e1fact / (1 + e1fact);
...
double CSamadi_Modified_Discrete_Model::eq_dZ(const double _T, const double _X) const
{
	// MOD: account for sleep quality

	// sleep quality always comes as a number between 0 and 1 (0 = not sleeping, 1 = sleeping well)
	// the idea is to basically converge to a level between Z_a (awake) and Z_s (sleeping) on the same scale, with the speed of f_Z

	const double sleep = mSleep_Quality.Get_Recent(_T * scgms::One_Minute);
	const double target = mParameters.Z_a + sleep * (mParameters.Z_s - mParameters.Z_a);

	return mParameters.f_Z * (_X - target);
}

void CSamadi_Modified_Discrete_Model::Emit_All_Signals(double time_advance_delta)
{
	...
	Emit_Signal_Level(samadi_modified_model::signal_Delivered_Insulin, _T, dosedinsulin);
	...
	Emit_Signal_Level(samadi_modified_model::signal_IOB, _T, iob);
	...
	Emit_Signal_Level(samadi_modified_model::signal_COB, _T, cob);
	...
	Emit_Signal_Level(samadi_modified_model::signal_BG, _T, bglevel);

	// IG - CGM
	const double iglevel = mState.Gsub;
	Emit_Signal_Level(samadi_modified_model::signal_IG, _T, iglevel);
}

HRESULT CSamadi_Modified_Discrete_Model::Do_Execute(scgms::UDevice_Event event) {
	HRESULT res = S_FALSE;
	...
	if (mState.lastTime > 0)
	{
		if (event.event_code() == scgms::NDevice_Event_Code::Level)
		{
			...
			else if (event.signal_id() == scgms::signal_Sleep_Quality)
			{
				if (event.device_time() < mState.lastTime)
					return E_ILLEGAL_STATE_CHANGE;
				...
				// TODO: is 10 minutes interval correct? This means, that if no sleep quality comes in 10 minutes, we take it as "no sleep info" and let the sleep submodel just slowly fade to its mean level
				mSleep_Quality.Add_Uptake(event.device_time(), event.device_time() + scgms::One_Minute * 10, event.level());
			}
...
HRESULT CSamadi_Modified_Discrete_Model::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
	// configured in the constructor - no need for additional configuration; signalize success
	return S_OK;
}

HRESULT IfaceCalling CSamadi_Modified_Discrete_Model::Step(const double time_advance_delta) {
	HRESULT rc = E_FAIL;
...
HRESULT CSamadi_Modified_Discrete_Model::Emit_Signal_Level(const GUID& signal_id, double device_time, double level) {
	scgms::UDevice_Event evt{ scgms::NDevice_Event_Code::Level };

	evt.device_id() = samadi_modified_model::model_id;
...
HRESULT IfaceCalling CSamadi_Modified_Discrete_Model::Initialize(const double current_time, const uint64_t segment_id) {
