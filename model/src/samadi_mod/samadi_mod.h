...
class CSamadi_Modified_Discrete_Model;

namespace samadi_modified_model
{
	using TDiff_Eq_Fnc = double (CSamadi_Modified_Discrete_Model::*)(const double, const double) const;
	using TDiff_Eq = decltype(std::bind<double>(std::declval<TDiff_Eq_Fnc>(), std::declval<CSamadi_Modified_Discrete_Model*>(), std::declval<const decltype(std::placeholders::_1)&>(), std::declval<const decltype(std::placeholders::_2)&>()));
...
// state of Samadi modified model equation system
struct CSamadi_Modified_Model_State
{
	double lastTime;
...
// original DOI: 10.1016/j.compchemeng.2019.106565
class CSamadi_Modified_Discrete_Model : public scgms::CBase_Filter, public scgms::IDiscrete_Model
{
	private:
	...
	private:
		samadi_modified_model::TParameters mParameters;
		...
		// heart rate storage (although technically not "uptake")
		Uptake_Accumulator mHeart_Rate;
		// sleep quality storage (although technically not "uptake")
		Uptake_Accumulator mSleep_Quality;
		// current state of Bergman model (all quantities)
		CSamadi_Modified_Model_State mState;
		// bound equations in a single vector - quantity and equation bound together
		const std::vector<samadi_modified_model::CEquation_Binding> mEquation_Binding;
	...
	private:
		...
		double eq_dZ(const double _T, const double _G) const;
	...
	public:
		CSamadi_Modified_Discrete_Model(scgms::IModel_Parameter_Vector *parameters, scgms::IFilter *output);
		virtual ~CSamadi_Modified_Discrete_Model() = default;
