...
class CGCT4_Discrete_Model : public scgms::CBase_Filter, public scgms::IDiscrete_Model {

	private:
		// last step time; NaN until initialized by outer code
		double mLast_Time = std::numeric_limits<double>::quiet_NaN();
		// model parameters
		gct4_model::TParameters mParameters;

		// all compartments present in system
		gct4_model::CGCT_Compartments mCompartments;

		// signals transformed to output signals, not yet emitted
		std::list<gct4_model::TPending_Signal> mPending_Signals;
	...
	public:
		CGCT4_Discrete_Model(scgms::IModel_Parameter_Vector* parameters, scgms::IFilter* output);
		virtual ~CGCT4_Discrete_Model() = default;
