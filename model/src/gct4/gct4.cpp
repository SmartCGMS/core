...
//#define GCT4_ENABLE_DEBUG_DRAWING

#ifdef GCT4_ENABLE_DEBUG_DRAWING
#include <scgms/utils/drawing/DebugDrawing.h>
debug_drawing::CCoupled_Drawing gDebugDrawing;
#endif

// this selects GCTv4 support implementations
using namespace gct4_model;
...
CGCT4_Discrete_Model::CGCT4_Discrete_Model(scgms::IModel_Parameter_Vector* parameters, scgms::IFilter* output) :
	CBase_Filter(output),
	...
	dc.Set_Persistent(true);
	dc.Set_Solution_Volume(1.0);
	dc.Set_Name(L"Dc");

	dp.Set_Persistent(true);
	dp.Set_Solution_Volume(1.0);
	dp.Set_Name(L"Dp");

	df.Set_Persistent(true);
...
void CGCT4_Discrete_Model::Emit_All_Signals(double time_advance_delta) {
	...
	Emit_Signal_Level(gct4_model::signal_COB, _T, cob * Glucose_Molar_Weight / (mParameters.Ag * 1000.0));
	...
	Emit_Signal_Level(gct4_model::signal_IOB, _T, iob);
	...
	Emit_Signal_Level(gct4_model::signal_BG, _T, bglevel);

	// IG - CGM
	const double iglevel = mCompartments[NGCT_Compartment::Glucose_Subcutaneous].Get_Concentration();
	Emit_Signal_Level(gct4_model::signal_IG, _T, iglevel);
}

HRESULT CGCT4_Discrete_Model::Do_Execute(scgms::UDevice_Event event) {
	HRESULT res = S_FALSE;
	...
	if (!std::isnan(mLast_Time)) {
		...
		if (event.event_code() == scgms::NDevice_Event_Code::Level) {
			...
			// meal absorption
				...
				Add_To_Gut_Staging(event.signal_id(), event.level(), event.device_time(), transferTime);
			...
			gDebugDrawing.Render_SVG("gct4.svg", 800, 600);
		}
#endif
	}
...
HRESULT CGCT4_Discrete_Model::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
	// configured in the constructor - no need for additional configuration; signalize success
	return S_OK;
}

HRESULT IfaceCalling CGCT4_Discrete_Model::Step(const double time_advance_delta) {
...
HRESULT CGCT4_Discrete_Model::Emit_Signal_Level(const GUID& signal_id, double device_time, double level) {

	scgms::UDevice_Event evt{ scgms::NDevice_Event_Code::Level };

	evt.device_id() = gct4_model::model_id;
...
HRESULT IfaceCalling CGCT4_Discrete_Model::Initialize(const double current_time, const uint64_t segment_id) {
	...
	if (std::isnan(mLast_Time)) {
		...
		if (mParameters.Dc_0 > 0) {
			Add_To_Gut_Staging(scgms::signal_Carb_Intake, mParameters.Dc_0, mLast_Time, 5_min);
		}
		if (mParameters.Dp_0 > 0) {
			Add_To_Gut_Staging(scgms::signal_Protein_Intake, mParameters.Dp_0, mLast_Time, 5_min);
		}
