...
CDb_Writer_Legacy::CDb_Writer_Legacy(scgms::IFilter *output) : CBase_Filter(output) {
	//
}

CDb_Writer_Legacy::~CDb_Writer_Legacy() {
	Flush_Levels();
}
...
int64_t CDb_Writer_Legacy::Create_Segment(std::wstring name, std::wstring comment) {
	auto qr = mDb_Connection.Query(rsLegacy_Db_Found_New_Segment, rsReserved_Segment_Name, L"", false);
...
int64_t CDb_Writer_Legacy::Create_Subject(std::wstring name) {
	auto qr = mDb_Connection.Query(rsLegacy_Db_Found_New_Subject, rsReserved_Subject_Name, L"", -1, 0);
	qr.Get_Next();
...
bool CDb_Writer_Legacy::Store_Level(const scgms::UDevice_Event& evt) {
...
void CDb_Writer_Legacy::Flush_Levels()
{
	// TODO: transactions
		...
		for (const auto& model : models) {
			for (size_t i = 0; i < model.number_of_calculated_signals; i++) {
				if (evt.signal_id() == model.calculated_signal_ids[i] && model.number_of_parameters == paramCnt) {
					...
					query_text = rsLegacy_Db_Insert_Params_Base;
...
HRESULT IfaceCalling CDb_Writer_Legacy::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
...
HRESULT IfaceCalling CDb_Writer_Legacy::Do_Execute(scgms::UDevice_Event event) {
...
HRESULT CDb_Writer_Legacy::Connect_To_Db_If_Needed()
{
	...
	switch (mSubject_Id)
	{
		...
		case db::New_Subject_Identifier:
			mSubject_Id = Create_Subject(db_writer_legacy::Subject_Base_Name);
			break;
		default:
