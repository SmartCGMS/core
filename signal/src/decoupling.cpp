...
HRESULT IfaceCalling CDecoupling_Filter::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
	...
	if (Is_Invalid_GUID(mDestination_Id)) {
		return E_INVALIDARG; //mSource_Id can be Invalid_GUID due to external reasons, such as malformed CSV log events
	}
	...
	if (condition_true) {
		...
		if (is_level) {
			...
			if (!stats.in_episode) {
				...
				if (std::isnan(stats.recent_device_time)) {
					stats.current_episode_start_time = event.device_time();
				}
				else {
					stats.current_episode_start_time = 0.5 * (stats.recent_device_time + event.device_time()); //see the reasoning below
				}
			}
