...
namespace pagmo2 {
	...
	//	a UDP must also be default, copy and move constructible.
		...
		TProblem(const solver::TSolver_Setup &setup, solver::TSolver_Progress &progress) : mSetup(setup), mProgress(progress) { 
			mBounded_Solution.resize(setup.size); 
		}
		...
		std::pair<pagmo::vector_double, pagmo::vector_double> get_bounds() const {
			pagmo::vector_double lb{ mSetup.lower_bound,  mSetup.lower_bound + mSetup.size};
			pagmo::vector_double ub{ mSetup.upper_bound, mSetup.upper_bound + mSetup.size};
			return { lb, ub };
		}
	};
...
public:
	...
	bool Solve(solver::TSolver_Progress &progress) {
		pagmo::vector_double champion_x(mSetup.size, std::numeric_limits<double>::quiet_NaN());
		bool succeded = false;
		...
		switch (mAlgo) {
			case pagmo2::EPagmo_Algo::PSO:		ps(pagmo::pso{ static_cast<unsigned int>(mSetup.max_generations) }); break;	
			case pagmo2::EPagmo_Algo::SADE:		ps(pagmo::sade{ static_cast<unsigned int>(mSetup.max_generations), 2, 1, mSetup.tolerance }); break;
			case pagmo2::EPagmo_Algo::DE1220:	ps(pagmo::de1220{ static_cast<unsigned int>(mSetup.max_generations), pagmo::de1220_statics<void>::allowed_variants, 1, mSetup.tolerance }); break;
			case pagmo2::EPagmo_Algo::ABC:		ps(pagmo::bee_colony{ static_cast<unsigned int>(mSetup.max_generations) });  break;
			case pagmo2::EPagmo_Algo::CMAES:	ps(pagmo::cmaes{ static_cast<unsigned int>(mSetup.max_generations), -1.0, -1.0, -1.0, -1.0, 0.5, mSetup.tolerance });  break;
			case pagmo2::EPagmo_Algo::xNES:		ps(pagmo::xnes{ static_cast<unsigned int>(mSetup.max_generations), -1.0, -1.0, -1.0, -1.0, mSetup.tolerance }); break;
		}
