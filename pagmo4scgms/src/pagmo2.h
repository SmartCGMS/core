...
namespace pagmo2
{
	...
	{
	protected:
		pagmo::vector_double mRemapped_solution, mRemapped_Lower, mRemapped_Upper;
		//must be initialized to the default solution as only the remapped elements will be replaced	
	...
	protected:
		...
		//just because Pagmo insists on default ctor
	...
	public:
		...
		TProblem(const solver::TSolver_Setup& setup, solver::TSolver_Progress& progress) : mSetup(setup), mRemap(setup),
			mProgress(progress)
		{
		}

		TProblem() :
			mSetup(solver::Default_Solver_Setup), mRemap(solver::Default_Solver_Setup), mProgress(mVoid_Progress)
		{
			//should not never occur, but default ctor is just required					
		};
		...
		pagmo::vector_double fitness(const pagmo::vector_double& x) const
		{
			...
			pagmo::vector_double result(solver::Maximum_Objectives_Count, std::numeric_limits<double>::quiet_NaN());
			//mSetup.objective may expect all the solver::TFitness
			mSetup.objective(mSetup.data, 1, solution.data(), result.data());
			//should it fail, result should remain set to nan 
...
protected:
	...
	std::tuple<bool, pagmo::vector_double> Execute_Solver(solver::TSolver_Progress& progress, TSolver&& solver)
	{
		...
		size_t isle_population = std::max(static_cast<size_t>(8),
		                                  mSetup.population_size / std::thread::hardware_concurrency());
		...
		if (mSetup.objectives_count > 1)
		{
			...
			{
				return scalar_fitnesses[a] < scalar_fitnesses[b];
			});
		}
		else
			std::sort(indexes.begin(), indexes.end(), [&](const size_t a, const size_t b)
			{
				return all_fitnesses[a][0] < all_fitnesses[b][0];
			});
...
public:
	...
	bool Solve(solver::TSolver_Progress& progress)
	{
		...
		switch (mAlgo)
		{
		case pagmo2::NPagmo_Algo::PSO: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::pso{static_cast<unsigned int>(mSetup.max_generations)});
			break;
		case pagmo2::NPagmo_Algo::SADE: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::sade{static_cast<unsigned int>(mSetup.max_generations), 2, 1, mSetup.tolerance});
			break;
		case pagmo2::NPagmo_Algo::DE1220: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::de1220{
					static_cast<unsigned int>(mSetup.max_generations), pagmo::de1220_statics<void>::allowed_variants, 1,
					mSetup.tolerance
				});
			break;
		case pagmo2::NPagmo_Algo::ABC: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::bee_colony{static_cast<unsigned int>(mSetup.max_generations)});
			break;
		case pagmo2::NPagmo_Algo::CMAES: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::cmaes{
					static_cast<unsigned int>(mSetup.max_generations), -1.0, -1.0, -1.0, -1.0, 0.5, mSetup.tolerance
				});
			break;
		case pagmo2::NPagmo_Algo::xNES: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::xnes{
					static_cast<unsigned int>(mSetup.max_generations), -1.0, -1.0, -1.0, -1.0, mSetup.tolerance
				});
			break;
		case pagmo2::NPagmo_Algo::GPSO: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::pso_gen{static_cast<unsigned int>(mSetup.max_generations)});
			break;

		case pagmo2::NPagmo_Algo::IHS: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::ihs{static_cast<unsigned int>(mSetup.max_generations)});
			break;
		case pagmo2::NPagmo_Algo::NSGA2: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::nsga2{static_cast<unsigned int>(mSetup.max_generations)});
			break;
		case pagmo2::NPagmo_Algo::MOEAD: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::moead{static_cast<unsigned int>(mSetup.max_generations), "random"});
			break;
		case pagmo2::NPagmo_Algo::MACO: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::maco{static_cast<unsigned int>(mSetup.max_generations)});
			break;
		case pagmo2::NPagmo_Algo::NSPSO: std::tie<bool, pagmo::vector_double>(succeeded, champion_x) = Execute_Solver(
				progress, pagmo::nspso{static_cast<unsigned int>(mSetup.max_generations)});
			break;
