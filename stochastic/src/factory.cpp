...
class CSolution_Dispatcher {
	...
	public:
		CSolution_Dispatcher() {
			...
			using TRandom_MetaDEv2_random_playback = CMetaDEv2<TUsed_Solution, CRandom_Playback>;
			mSolver_Id_Map[rnd_metadev2_random_playback::id] = std::bind(&Solve_By_Class<TRandom_MetaDEv2_random_playback, TUsed_Solution>, std::placeholders::_1, std::placeholders::_2);
	...
	public:
		HRESULT do_solve(const GUID &solver_id, solver::TSolver_Setup &setup, solver::TSolver_Progress &progress) {
			...
			switch (setup.problem_size) {
				case 2:
					return call_the_solver(mDispatcher_2);
				case 4:
					return call_the_solver(mDispatcher_4);
				case 6:
					return call_the_solver(mDispatcher_6);
				case 8:
					return call_the_solver(mDispatcher_8);
				default:
					return call_the_solver(mDispatcher_General);
			}

			//just to keep the compiler happy as we never get here
			return E_FAIL;
		}
};
