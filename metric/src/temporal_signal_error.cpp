...
CTemporal_Signal_Error::~CTemporal_Signal_Error() {
	if (mPromised_Metric) {
...
HRESULT CTemporal_Signal_Error::On_Level_Added(const uint64_t segment_id, const double device_time) {
	HRESULT rc = S_OK;
...
HRESULT CTemporal_Signal_Error::Do_Execute(scgms::UDevice_Event event) {
...
HRESULT CTemporal_Signal_Error::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
	...
	if (Is_Invalid_GUID(temporal_metric_id)) {
		std::wstring err_desc = dsInvalid_Metric_GUID;
		err_desc += configuration.Read_String(temporal_signal_error::rsTemporal_Metric);
		error_description.push(err_desc);
		return E_INVALIDARG;
	}
	...
	mAllow_Multipoint_Affinity = configuration.Read_Bool(temporal_signal_error::rsAllow_Multipoint_Affinity, mAllow_Multipoint_Affinity);
	...
	const scgms::TMetric_Parameters temporal_metric_parameters{ temporal_metric_id,
		bool_2_uc(configuration.Read_Bool(rsUse_Relative_Error)),
		bool_2_uc(configuration.Read_Bool(rsUse_Squared_Diff)),
		bool_2_uc(configuration.Read_Bool(rsUse_Prefer_More_Levels)),
		metric_threshold
	...
	mTemporal_Metric = scgms::SMetric{ temporal_metric_parameters };
...
double CTemporal_Signal_Error::Calculate_Metric(const uint64_t segment_id) {
	...
	if (Prepare_Unaligned_Discrete_Levels(segment_id, reference_times, reference_levels, error_times, error_levels, mAllow_Multipoint_Affinity)) {
		...
		std::vector<double> scaled_ref = reference_times;
		std::vector<double> scaled_err = error_times;

		for (size_t i = 0; i < scaled_ref.size(); i++) {
		...
		std::transform(scaled_err.begin(), scaled_err.end(), scaled_err.begin(), [smin, smax, lev_smin, lev_smax](const double val) {
			return lev_smin + (lev_smax - lev_smin) * (val - smin) / (smax - smin);
		});

		std::fill(scaled_ref.begin(), scaled_ref.end(), 0);

		if (mTemporal_Metric->Reset() == S_OK) {
...
HRESULT IfaceCalling CTemporal_Signal_Error::Calculate_Signal_Error(const uint64_t segment_id, scgms::TSignal_Stats *absolute_error, scgms::TSignal_Stats *relative_error) {
	...
	std::lock_guard<std::mutex> lock{ mSeries_Gaurd };
	if (Prepare_Unaligned_Discrete_Levels(segment_id, times, reference_levels, error_times, error_levels, mAllow_Multipoint_Affinity)) {
